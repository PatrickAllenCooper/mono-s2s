...
# Keys are substrings matched against the SLURM job name.
# Handles both legacy names (foundation_monotonic) and seed-specific names
# (foundation_s3_seed42) generated by submit_all_seeds.sh.
declare -A JOB_SCRIPT_MAP=(
    ["foundation_setup"]="jobs/job_0_setup.sh"
    ["s0_seed"]="jobs/job_0_setup.sh"
    ["foundation_monotonicity"]="jobs/job_1_monotonicity.sh"
    ["s1_seed"]="jobs/job_1_monotonicity.sh"
    ["foundation_baseline"]="jobs/job_2_baseline.sh"
    ["s2_seed"]="jobs/job_2_baseline.sh"
    ["foundation_monotonic"]="jobs/job_3_monotonic.sh"
    ["s3_seed"]="jobs/job_3_monotonic.sh"
    ["foundation_eval"]="jobs/job_4_evaluate.sh"
    ["s4_seed"]="jobs/job_4_evaluate.sh"
    ["foundation_uat"]="jobs/job_5_uat.sh"
    ["s5_seed"]="jobs/job_5_uat.sh"
    ["foundation_hotflip"]="jobs/job_6_hotflip.sh"
    ["s6_seed"]="jobs/job_6_hotflip.sh"
    ["foundation_aggregate"]="jobs/job_7_aggregate.sh"
    ["s7_seed"]="jobs/job_7_aggregate.sh"
)
...
resubmit_job() {
    ...
    # Extract seed from job name (e.g. "foundation_s3_seed42" -> "42")
    local seed_suffix=""
    if [[ "$job_name" =~ seed([0-9]+) ]]; then
        seed_suffix="${BASH_REMATCH[1]}"
    fi

    # Build --export string, preserving seed and variant
    local export_str="ALL"
    if [ -n "$seed_suffix" ]; then
        export_str="ALL,EXPERIMENT_SEED=${seed_suffix},MONOTONIC_VARIANT=${MONOTONIC_VARIANT:-mlp_both}"
    fi

    # Resubmit
    local new_job_id=$(sbatch --parsable --export="$export_str" "$job_script" 2>&1 | grep -oE '[0-9]{7,}' | tail -1)
