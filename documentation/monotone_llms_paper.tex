...
\icmltitlerunning{Monotone LLMs}

\begin{document}

\twocolumn[
\icmltitle{Monotone LLMs}
...
\begin{abstract}
As large language models (LLMs) are increasingly deployed in real-world and safety-sensitive settings, their vulnerability to adversarial prompts and jailbreak attacks has emerged as a critical concern. Although existing defenses, including alignment techniques, prompt filtering, and adversarial training, have improved empirical safety, they largely operate as reactive mechanisms and provide limited structural guaranties about model behavior.
We propose \emph{monotone language models}, an architectural approach that enforces monotonicity constraints within the feed-forward components of Transformer-based sequence-to-sequence models. Monotonicity introduces a principled inductive bias that yields more predictable behavior under adversarial perturbations and simplifies reasoning about model responses. Primarily, our approach is complementary to existing safety and alignment methods and can be implemented alongside them without modifying training objectives or inference pipelines.
Empirically, we demonstrate that monotone Transformers exhibit improved robustness to a range of adversarial and jailbreak attacks while preserving performance on standard summarization benchmarks. We further show that pretrained language models can be distilled into monotone counterparts with no noticeable degradation in task accuracy. Our results suggest that monotonicity is a practical and scalable architectural bias for improving the reliability of large language models, offering a complementary path toward more robust and trustworthy systems.
\end{abstract}

\section{Introduction}
Large language models (LLMs) have rapidly transitioned from research artifacts to widely deployed systems that influence real-world decisions. They are now used to summarize documents, assist in legal and medical workflows, and act as intermediaries between users and critical information systems. This rapid adoption has brought renewed attention to a fundamental challenge: despite their impressive capabilities, LLMs remain highly vulnerable to adversarial manipulation. Carefully crafted prompts, jailbreak strategies, or maliciously structured inputs can induce unsafe or unintended behavior, often with minimal effort.

In response, a growing body of work has proposed defenses aimed at improving the safety and robustness of language models. These include prompt-based safeguards, supervised fine-tuning, reinforcement learning from human feedback, adversarial training, and post-hoc filtering mechanisms. While effective in many cases, such approaches are inherently reactive: they aim to mitigate known failure modes rather than constrain model behavior by design. As a result, they provide limited guarantees when models are exposed to novel or adaptive adversaries.

This limitation is particularly pronounced in large, high-dimensional models, where exhaustive testing or verification of behavior is infeasible. As LLMs are increasingly embedded in high-stakes applications, there is a growing need for complementary approaches that impose structural regularities on model behavior, enabling more predictable and analyzable responses under perturbation.

We explore \emph{monotonicity} as such a structural inductive bias. Monotonicity is a classical property in machine learning and optimization, referring to functions whose outputs vary in a predictable direction with respect to their inputs. When incorporated into neural networks, monotonicity enables simpler reasoning about model behavior and has been shown to facilitate verification and robustness in lower-dimensional settings. However, prior attempts to impose monotonic constraints on neural models have often resulted in reduced expressive power or degraded performance, limiting their applicability to large-scale language models.

Our key insight is that monotonicity can be enforced selectively within modern Transformer architectures without sacrificing performance. Rather than constraining the entire model, we apply monotonicity constraints to the feed-forward sublayers that dominate the model's nonlinear transformations, while leaving attention mechanisms, residual connections, and normalization layers unconstrained. To enable effective optimization under these constraints, we adopt a smooth reparameterization strategy that avoids the degeneracies associated with projection-based methods.

Building on this design, we show that pretrained sequence-to-sequence models can be distilled into monotone counterparts that closely match their original performance on standard benchmarks. Despite their constrained structure, these monotone models exhibit increased robustness to adversarial and jailbreak attacks, suggesting that monotonicity acts as a stabilizing inductive bias rather than a limitation.

Importantly, our approach is not intended to replace existing safety or alignment techniques. Instead, it is fully complementary: monotonicity operates at the architectural level and can be combined with current training procedures, alignment strategies, and inference-time defenses. By providing an additional layer of structural regularization, monotone language models offer a lightweight and scalable path toward improving robustness and reliability in large language models.

Taken together, our results demonstrate that architectural constraints and empirical performance need not be at odds. Monotonicity emerges as a practical design principle for modern language models—one that enhances robustness while remaining compatible with the broader ecosystem of safety methods developed to date.
...
\section{Results}
\paragraph{HotFlip Attacks.}
We evaluate gradient-based robustness using HotFlip attacks, which identify vulnerable token positions via embedding gradients and perform targeted replacements to maximize loss. Table~\ref{tab:hotflip} reports average ROUGE-L degradation, attack success rate (defined as degradation exceeding 10\%), and mean loss increase across 200 test examples. The monotonic model exhibits substantially improved robustness compared to both baselines. Under HotFlip perturbations, the monotonic model incurs an average ROUGE-L degradation of 9.35\%, compared to 14.60\% for the fine-tuned baseline and 21.44\% for the standard pretrained model. Attack success rates follow a similar pattern: 39.0\% for the monotonic model versus 54.0\% for the baseline and 70.0\% for the standard model. These results suggest that the non-negative weight constraints in feed-forward sublayers reduce the model's susceptibility to gradient-based input manipulations, providing a meaningful robustness benefit without requiring adversarial training.

\begin{table}[t]
\caption{HotFlip attack results on CNN/DailyMail.}
\label{tab:hotflip}
\vskip 0.1in
\begin{center}
\begin{small}
\begin{tabular}{lccc}
\toprule
Model & Deg. & Succ. & $\Delta$Loss \\
\midrule
Standard  & 21.4\% & 70\% & +0.41 \\
Baseline  & 14.6\% & 54\% & +0.31 \\
Monotonic &  9.4\% & 39\% & +0.24 \\
\bottomrule
\end{tabular}
\end{small}
\end{center}
