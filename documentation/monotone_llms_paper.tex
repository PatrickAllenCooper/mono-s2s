...
\textcolor{blue}{We use T5-small, which consists of 6 encoder layers and 6 decoder layers with 512-dimensional hidden states, 8 attention heads per layer, and 2048-dimensional FFN intermediate activations. The model contains approximately 60M parameters total, of which 24M reside in the FFN projection matrices that we subject to monotonicity constraints, representing 40 percent of model parameters. Monotonicity constraints are applied to the FFN projections in all 12 Transformer blocks, yielding 24 constrained weight matrices. All other components remain unconstrained, including attention projections, layer normalization, embedding layers, and the output projection layer.}

\subsection{Training and Evaluation Protocol}
Both baseline and monotone models are fine-tuned from the same pretrained T5-small checkpoint under identical optimization settings. We use the AdamW optimizer with learning rate $5 \times 10^{-5}$ and weight decay $0.01$, a batch size of 4, and gradient clipping with norm 1.0. The baseline model is trained for 5 epochs, while the monotone model is trained for 7 epochs with an extended warmup phase (15\% of training steps, compared to 10\% for the baseline) to accommodate the constrained parameter space.

At inference time, decoding is performed using beam search with 4 beams and a length penalty of 1.2. Models are trained on DialogSum, HighlightSum, and arXiv abstract datasets (approximately 50K examples each), and evaluated on CNN/DailyMail using ROUGE-1, ROUGE-2, and ROUGE-L. We report bootstrap-based 95\% confidence intervals for all metrics.

All reported results are obtained using T5-small and evaluation subsets of 200 examples. We view scaling to larger model variants (e.g., T5-base and T5-large) and full test sets as an important direction for future work.

\textcolor{blue}{For fair comparison, both baseline and monotonic models are trained for 7 epochs under identical optimization settings. The monotonic model uses an extended warmup phase (15 percent of training steps compared to 10 percent for baseline) to accommodate the softplus parameterization, but both models receive identical total training budgets. We train on DialogSum, HighlightSum, and arXiv abstract datasets with a combined training corpus of approximately 150K examples. We evaluate on three held-out benchmarks: CNN/DailyMail (11,490 test examples), XSUM (11,334 test examples), and SAMSum (819 test examples). CNN/DailyMail is held out entirely from training to measure out-of-distribution generalization.}

\textcolor{blue}{At inference time, we use beam search with 4 beams, length penalty 1.2, minimum generation length 10 tokens, maximum generation length 80 tokens, and no-repeat n-gram blocking with n equal to 3. These parameters are fixed across all models and evaluation sets. ROUGE scores are computed using the rouge-score library with Porter stemming. We report bootstrap 95 percent confidence intervals using 1,000 resamples. Statistical significance is assessed using paired t-tests with Bonferroni correction for multiple comparisons. We additionally report Cohen's d effect sizes.}

\textcolor{blue}{To assess robustness of our findings to training stochasticity, we repeat all experiments with 5 random seeds (42, 1337, 2024, 8888, 12345) controlling Python, NumPy, and PyTorch random number generators. We report mean and standard deviation across seeds for all metrics. Statistical significance across training runs is assessed using paired t-tests on seed-level means with n equal to 5 observations per condition, in addition to per-example paired t-tests within each seed. This dual approach captures both evaluation variance (differences on the same test examples) and training variance (differences due to random initialization and data ordering). All experiments use deterministic algorithms where available. Training is conducted on a single NVIDIA A100 GPU with 40GB memory using PyTorch 2.0.1 and Transformers 4.30.2.}
...
\textcolor{blue}{For UAT attacks, we optimize triggers by minimizing the negative log-likelihood objective over validation examples and evaluate on 1,500 disjoint test examples. Candidate tokens are drawn from a vocabulary of 1,000 tokens biased toward punctuation, special characters, and high-frequency words that empirically cause greater disruption. For HotFlip attacks, we compute gradients of the loss with respect to input token embeddings, identify the top 5 positions with largest gradient magnitudes, and replace tokens at those positions with vocabulary items that maximize the dot product between the gradient and the replacement embedding. Attack success rate is computed as the fraction of examples where ROUGE-L degradation exceeds 10 percentage points relative to clean performance. We additionally report mean loss increase and construct a transfer matrix to assess cross-model attack transferability.}

\section{Results}

\subsection{Training Dynamics and Summarization Quality}

\textcolor{blue}{The monotonicity constraints impose a distinct optimization trajectory compared to unconstrained fine-tuning. Table~\ref{tab:training} summarizes the training dynamics for both models. The monotonic model begins training with substantially higher initial loss (4.97 vs 2.90 for baseline), reflecting the immediate impact of the softplus reparametrization on the pretrained weight distribution. Taking absolute values and applying the inverse softplus transformation distorts the original weight geometry, requiring the model to recover effective representations during fine-tuning.}

\textcolor{blue}{Despite this initialization gap, the monotonic model demonstrates consistent convergence across 7 epochs, with validation loss decreasing from 2.92 to 2.54. However, a persistent optimization gap remains: the monotonic model converges to a validation loss approximately 0.29 higher than the baseline (2.54 vs 2.25). This gap reflects the reduced expressivity of the constrained parameter space, where all FFN weights are restricted to non-negative values.}

\begin{table}[t]
\caption{\textcolor{blue}{Training dynamics comparison (seed=42).}}
\label{tab:training}
\vskip 0.1in
\begin{center}
\begin{small}
...
\midrule
Baseline  & 2.90 & 2.47 & $-$0.43 & 5 \\
Monotonic & 4.97 & 2.69 & $-$2.28 & 7 \\
\bottomrule
\end{tabular}
\end{small}
\end{center}
...
\textcolor{blue}{Table~\ref{tab:rouge} reports summarization quality on CNN/DailyMail. The monotonic model achieves ROUGE-L of 24.2 compared to 25.0 for baseline, a relative decrease of 3.2 percent. ROUGE-1 shows a similar pattern (29.6 vs 30.9, a 4.2 percent decrease). These results indicate that the monotonicity constraints impose a modest but measurable cost to task performance. Importantly, the monotonic model remains competitive: it achieves ROUGE scores within the 95 percent confidence intervals of the baseline for ROUGE-2 and ROUGE-L, suggesting the quality degradation may not be statistically significant in all metrics.}

\begin{table}[t]
\caption{\textcolor{blue}{Summarization quality on CNN/DailyMail (n=200, seed=42). Values are mean with 95\% bootstrap CI.}}
\label{tab:rouge}
\vskip 0.1in
\begin{center}
\begin{small}
...
\textcolor{blue}{The training dynamics reveal a characteristic pattern of monotonically-constrained optimization: rapid initial recovery followed by slower asymptotic convergence. During the first two epochs, the monotonic model reduces training loss by 1.92 points (4.97 to 3.05), recovering most of the performance gap introduced by the constraint initialization. Subsequent epochs yield diminishing returns, with only 0.36 points of improvement across epochs 3 through 7. This pattern suggests that the extended warmup ratio (15 percent vs 10 percent for baseline) is essential for stable optimization, allowing gradient estimates to stabilize before aggressive weight updates.}

\textcolor{blue}{Notably, the monotonic model exhibits lower variance in prediction lengths compared to the standard pretrained model (std 11.3 tokens vs 12.1 tokens), suggesting that the non-negative weight constraints may induce more consistent generation behavior. Both fine-tuned models produce longer outputs than the standard T5 (mean 74-76 tokens vs 57 tokens), indicating that task-specific fine-tuning adjusts the models length prior toward the training distribution.}

\subsection{Adversarial Robustness}

\paragraph{HotFlip Attacks.}
We evaluate gradient-based robustness using HotFlip attacks, which identify vulnerable token positions via embedding gradients and perform targeted replacements to maximize loss. For each input-summary pair, the attack computes the gradient of the cross-entropy loss with respect to input token embeddings, identifies the 5 positions with largest gradient magnitudes, and replaces tokens at those positions with vocabulary items that maximize the dot product between the gradient and the replacement embedding. This greedy token substitution is designed to maximally increase the model's prediction loss.

\textcolor{blue}{We define three robustness metrics. First, average degradation measures the mean relative loss increase under attack, computed as $\frac{1}{n}\sum_{i=1}^{n} \frac{\mathcal{L}(x_i^{\text{attack}}, y_i) - \mathcal{L}(x_i, y_i)}{\mathcal{L}(x_i, y_i)}$, where $\mathcal{L}$ denotes cross-entropy loss, $x_i$ is the original input, $x_i^{\text{attack}}$ is the perturbed input, and $y_i$ is the reference summary. Second, attack success rate is the fraction of examples where degradation exceeds 10 percent, formally $\frac{1}{n}\sum_{i=1}^{n} \mathbf{1}[\text{degradation}_i > 0.10]$. This threshold captures attacks that meaningfully disrupt model predictions rather than causing negligible perturbations. Third, mean loss increase ($\Delta$Loss) reports the average absolute difference in loss, $\frac{1}{n}\sum_{i=1}^{n} [\mathcal{L}(x_i^{\text{attack}}, y_i) - \mathcal{L}(x_i, y_i)]$, providing an unnormalized measure of attack impact.}

Table~\ref{tab:hotflip} reports these metrics across 100 test examples. The monotonic model exhibits substantially improved robustness compared to both baselines. Under HotFlip perturbations, the monotonic model incurs an average degradation of 5.2\%, compared to 16.3\% for the fine-tuned baseline and 21.4\% for the standard pretrained model. Attack success rates follow a similar pattern: 19\% for the monotonic model versus 61\% for the baseline and 70\% for the standard model. Statistical testing confirms highly significant differences between baseline and monotonic models ($t=6.28$, $p<10^{-9}$). These results suggest that the non-negative weight constraints in feed-forward sublayers reduce the model's susceptibility to gradient-based input manipulations, providing a 68\% reduction in attack success rate without requiring adversarial training.
...
\textcolor{blue}{The monotonic model demonstrates a 68\% relative reduction in attack success rate compared to baseline (19\% vs 61\%) and a 68\% reduction in average ROUGE-L degradation (5.2\% vs 16.3\%). Effect size analysis reveals a large effect (Cohen's $d > 0.8$) for the baseline vs monotonic comparison. The mean loss increase under attack is also substantially lower for the monotonic model (+0.13) compared to baseline (+0.35), indicating that adversarial perturbations have reduced impact on model predictions.}

\paragraph{Universal Adversarial Trigger Attacks.}
\textcolor{blue}{TODO: UAT results pending with improved attack parameters (100 iterations, 5 restarts, 1000 optimization samples). Preliminary results showed limited trigger effectiveness; updated experiments will provide more meaningful comparison.}
