...
We use T5-small~\citep{raffel2020exploring}, which consists of 6 encoder layers and 6 decoder layers with 512-dimensional hidden states, 8 attention heads per layer, and 2048-dimensional FFN intermediate activations. The model contains approximately 60M parameters total, of which 24M reside in the FFN projection matrices (\texttt{wi}, \texttt{wo}) that we subject to monotonicity constraints. This represents 40\% of model parameters.

Monotonicity constraints are applied to the FFN projections in all 12 Transformer blocks (6 encoder + 6 decoder), yielding 24 constrained weight matrices. All other components remain unconstrained: attention Q/K/V projections (36M parameters), layer normalization, embedding layers, and the output projection layer.}

\subsection{Training and Evaluation Protocol}

\textcolor{blue}{\paragraph{Training Data.}
We train on three publicly available summarization datasets:
\begin{itemize}
\item \textbf{DialogSum}~\citep{TODO}: dialogue summarization (train: \textbf{TODO} examples, validation: \textbf{TODO} examples)
\item \textbf{HighlightSum}~\citep{TODO}: highlight extraction (train: \textbf{TODO} examples, validation: \textbf{TODO} examples)
\item \textbf{arXiv}~\citep{TODO}: scientific paper abstracts (train: \textbf{TODO} examples, validation: \textbf{TODO} examples)
\end{itemize}
Total training corpus: \textbf{TODO} examples. All datasets use standard train/validation splits. Texts are tokenized using T5Tokenizer with maximum input length 512 tokens and maximum target length 128 tokens. We combine datasets by simple concatenation without sampling or weighting.}

\textcolor{blue}{\paragraph{Evaluation Data.}
We evaluate on three held-out summarization benchmarks:
\begin{itemize}
\item \textbf{CNN/DailyMail v3.0.0}~\citep{TODO}: news article summarization, test split (11,490 examples)
\item \textbf{XSUM}~\citep{TODO}: extreme summarization, test split (11,334 examples)
\item \textbf{SAMSum}~\citep{TODO}: dialogue summarization, test split (819 examples)
\end{itemize}
Critically, CNN/DailyMail is held out entirely from training to ensure evaluation measures out-of-distribution generalization. XSUM and SAMSum appear in training (train splits) and evaluation (test splits), but splits are strictly disjoint.}

\paragraph{Optimization Settings.}
Both baseline and monotone models are fine-tuned from the same pretrained T5-small checkpoint under identical optimization settings. We use the AdamW optimizer with learning rate $5 \times 10^{-5}$ and weight decay $0.01$, a batch size of 4, and gradient clipping with norm 1.0. \textcolor{blue}{Both models train for 7 epochs to ensure fair comparison. The monotone model uses an extended warmup phase (15\% of training steps, compared to 10\% for the baseline) to accommodate the softplus parameterization, but both models receive identical total training budgets.}
...
All experiments use fixed random seeds (Python=42, NumPy=42, PyTorch=42) with deterministic algorithms enabled where available. We disable TF32 operations on Ampere GPUs and use CUDA deterministic mode for numerical reproducibility. Training is conducted on a single NVIDIA A100 GPU (40GB memory). We use PyTorch 2.0.1, Transformers 4.30.2, and Python 3.10. Training requires approximately \textbf{TODO} hours per model (7 epochs). Code and model checkpoints will be made available upon acceptance.}
...
Table~\ref{tab:clean} reports ROUGE scores on non-adversarial test data. All three models achieve strong summarization performance, with the fine-tuned models (Baseline and Monotonic) substantially outperforming the standard pretrained model. Critically, the monotonic model achieves \textbf{TODO} ROUGE-L, matching \textbf{TODO}\% of the baseline's performance (\textbf{TODO} ROUGE-L) despite the architectural constraints imposed on 40\% of model parameters.

Statistical testing reveals \textbf{TODO: [no significant difference / small significant difference]} between Baseline and Monotonic on clean performance (p=\textbf{TODO}, Cohen's d=\textbf{TODO}), demonstrating that monotonicity constraints preserve task capability while providing robustness benefits.
...
\midrule
Standard  & \textbf{TODO} $\pm$ \textbf{TODO} & \textbf{TODO} $\pm$ \textbf{TODO} & \textbf{TODO} $\pm$ \textbf{TODO} \\
Baseline  & \textbf{TODO} $\pm$ \textbf{TODO} & \textbf{TODO} $\pm$ \textbf{TODO} & \textbf{TODO} $\pm$ \textbf{TODO} \\
Monotonic & \textbf{TODO} $\pm$ \textbf{TODO} & \textbf{TODO} $\pm$ \textbf{TODO} & \textbf{TODO} $\pm$ \textbf{TODO} \\
\bottomrule
\end{tabular}
\end{small}
\end{center}
...
We evaluate gradient-based robustness using HotFlip attacks, which identify vulnerable token positions via embedding gradients and perform targeted replacements to maximize loss. Table~\ref{tab:hotflip} reports average ROUGE-L degradation, attack success rate (defined as degradation exceeding 10\%), and mean loss increase across \textcolor{blue}{1,500} test examples. The monotonic model exhibits substantially improved robustness compared to both baselines. Under HotFlip perturbations, the monotonic model incurs an average ROUGE-L degradation of \textcolor{blue}{\textbf{TODO}}\%, compared to \textcolor{blue}{\textbf{TODO}}\% for the fine-tuned baseline and \textcolor{blue}{\textbf{TODO}}\% for the standard pretrained model. Attack success rates follow a similar pattern: \textcolor{blue}{\textbf{TODO}}\% for the monotonic model versus \textcolor{blue}{\textbf{TODO}}\% for the baseline and \textcolor{blue}{\textbf{TODO}}\% for the standard model (p\textcolor{blue}{<0.01, Cohen's d=\textbf{TODO}}). These results suggest that the non-negative weight constraints in feed-forward sublayers reduce the model's susceptibility to gradient-based input manipulations, providing a meaningful robustness benefit without requiring adversarial training.
...
\midrule
Standard  & \textcolor{blue}{\textbf{TODO}}\% & \textcolor{blue}{\textbf{TODO}}\% & \textcolor{blue}{+\textbf{TODO}} \\
Baseline  & \textcolor{blue}{\textbf{TODO}}\% & \textcolor{blue}{\textbf{TODO}}\% & \textcolor{blue}{+\textbf{TODO}} \\
Monotonic & \textcolor{blue}{\textbf{TODO}}\% & \textcolor{blue}{\textbf{TODO}}\% & \textcolor{blue}{+\textbf{TODO}} \\
\bottomrule
\end{tabular}
\end{small}
\end{center}
...
Table~\ref{tab:uat} reports results under universal adversarial triggers. Consistent with HotFlip findings, the monotonic model demonstrates substantially improved robustness to input-agnostic adversarial prefixes. The monotonic model exhibits \textbf{TODO}\% lower attack success rate compared to baseline (p\textbf{<TODO}, Cohen's d=\textbf{TODO}), demonstrating robustness to both gradient-based (HotFlip) and gradient-free (UAT) attack strategies.
...
\midrule
Standard  & \textbf{TODO} & \textbf{TODO} & -\textbf{TODO}\% & \textbf{TODO}\% \\
Baseline  & \textbf{TODO} & \textbf{TODO} & -\textbf{TODO}\% & \textbf{TODO}\% \\
Monotonic & \textbf{TODO} & \textbf{TODO} & -\textbf{TODO}\% & \textbf{TODO}\% \\
\bottomrule
\end{tabular}
\end{small}
\end{center}
...
\midrule
Standard  & \textbf{TODO}\% & \textbf{TODO}\% & \textbf{TODO}\% \\
Baseline  & \textbf{TODO}\% & \textbf{TODO}\% & \textbf{TODO}\% \\
Monotonic & \textbf{TODO}\% & \textbf{TODO}\% & \textbf{TODO}\% \\
\bottomrule
\end{tabular}
\end{small}
\end{center}
...
To assess whether robustness improvements generalize beyond CNN/DailyMail, we evaluate on two additional summarization benchmarks: XSUM (extreme summarization of news) and SAMSum (dialogue summarization). Table~\ref{tab:multidataset} shows attack success rates across all three datasets under HotFlip attacks with 5 token replacements.

The monotonic model demonstrates consistent robustness advantages across all domains, with attack success rate reductions of \textbf{TODO}\% (CNN/DM), \textbf{TODO}\% (XSUM), and \textbf{TODO}\% (SAMSum) relative to baseline. This consistency indicates that monotonicity provides domain-general robustness improvements rather than overfitting to the specific characteristics of CNN/DailyMail.
...
\midrule
CNN/DM   & \textbf{TODO}\% & \textbf{TODO}\% & -\textbf{TODO}\% \\
XSUM     & \textbf{TODO}\% & \textbf{TODO}\% & -\textbf{TODO}\% \\
SAMSum   & \textbf{TODO}\% & \textbf{TODO}\% & -\textbf{TODO}\% \\
\midrule
Average  & \textbf{TODO}\% & \textbf{TODO}\% & -\textbf{TODO}\% \\
\bottomrule
\end{tabular}
\end{small}
\end{center}
...
Table~\ref{tab:cost} reports computational requirements for training and inference. The softplus parameterization introduces minimal overhead: training time increases by \textbf{TODO}\%, inference latency by \textbf{TODO}\%, while peak memory usage remains comparable. This demonstrates that monotonicity constraints are practical for production deployment and do not impose prohibitive computational costs.
...
\midrule
Baseline  & \textbf{TODO} hrs & \textbf{TODO} & \textbf{TODO} GB \\
Monotonic & \textbf{TODO} hrs & \textbf{TODO} & \textbf{TODO} GB \\
\midrule
Overhead  & +\textbf{TODO}\%  & +\textbf{TODO}\% & +\textbf{TODO}\% \\
\bottomrule
\end{tabular}
\end{small}
\end{center}
...
Our results demonstrate that monotonicity constraints substantially improve adversarial robustness while preserving task performance on clean data. The monotonic model achieves \textbf{TODO}\% of baseline clean performance (Table~\ref{tab:clean}) while reducing attack success rates by \textbf{TODO} percentage points on average across attack types (Tables~\ref{tab:hotflip} and~\ref{tab:uat}).
...
\textbf{TODO: [After running gradient analysis]} We hypothesize that non-negative weights reduce the magnitude of input gradients $\|\nabla_x \ell\|$, making gradient-based attacks inherently less effective. Preliminary analysis suggests monotonic models exhibit \textbf{TODO}\% lower average gradient norms compared to baseline, though comprehensive evaluation is needed to confirm this mechanism.
