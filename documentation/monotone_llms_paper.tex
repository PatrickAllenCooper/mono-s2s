...
Table~\ref{tab:rouge} reports summarization performance on CNN/DailyMail. The monotonic model achieves a ROUGE-L score of 24.2, compared to 25.0 for the baseline, corresponding to a relative decrease of 3.2\%. ROUGE-1 exhibits a similar trend (29.6 vs.\ 30.9, a 4.2\% decrease). While these results indicate a modest reduction in task performance, the monotonic model remains competitive: its ROUGE-2 and ROUGE-L scores fall within the 95\% confidence intervals of the baseline, suggesting that the degradation is not statistically significant across all metrics.
...
Table~\ref{tab:hotflip} reports results on 100 test examples from our completed experiment (seed 42). The monotonic model demonstrates substantially improved robustness relative to both baselines. Under HotFlip perturbations, the monotonic model incurs an average degradation of 5.2 percent, compared to 16.1 percent for the fine-tuned baseline and 21.4 percent for the standard pretrained model. Attack success rates follow a similar pattern, with only 19 percent of attacks succeeding against the monotonic model, compared to 63 percent for the baseline and 70 percent for the standard model. This represents a 69.8 percent relative reduction in attack success rate and a 67.8 percent reduction in degradation magnitude. Statistical testing using paired t-tests confirms that the difference between the baseline and monotonic models is highly significant ($t = 6.17$, $p < 10^{-9}$), and the difference between standard and monotonic models is even more pronounced ($t = 7.39$, $p < 10^{-12}$).
...
Overall, enforcing monotonicity in the FFN sublayers yields a 69.8 percent relative reduction in attack success rate (63 percent to 19 percent) and a 67.8 percent reduction in average degradation (16.1 percent to 5.2 percent), without requiring adversarial training, modified training objectives, or architectural changes beyond the FFN constraints. The effect size is large (Cohen's d > 0.8) and the statistical significance is robust ($p < 10^{-9}$). These results demonstrate that structural constraints alone—applied selectively to feed-forward sublayers—can substantially improve robustness to gradient-based adversarial manipulation, providing evidence for monotonicity as a practical architectural inductive bias for building more robust language models.

\paragraph{Universal Adversarial Trigger Attacks.}
We additionally evaluate robustness to universal adversarial triggers (UATs)—short, input-agnostic token sequences optimized to maximize model loss when prepended to any input. Triggers are learned via coordinate ascent with 3 restarts and 50 iterations over 80 training examples, then evaluated on 120 disjoint test examples. We measure robustness via NLL increase under trigger prepending.
