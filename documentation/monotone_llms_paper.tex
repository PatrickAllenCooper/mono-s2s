...
We begin by analyzing the effect of monotonicity constraints on optimization behavior and summarization quality. Table~\ref{tab:training} summarizes the training dynamics of the baseline and monotonic models from our completed experiment (seed 42). The monotonic model starts with a substantially higher initial loss (4.97 vs.\ 2.90), reflecting the impact of the softplus reparameterization and the transformation of pretrained weights into a non-negative space. This initialization disrupts the original weight geometry, requiring the model to re-establish effective internal representations during fine-tuning.

Despite this unfavorable initialization, the monotonic model converges reliably over the course of training. Validation loss decreases from 2.92 at epoch 1 to 2.54 at epoch 7, indicating stable optimization under the imposed constraints. The model exhibits rapid initial recovery—reducing training loss by 1.92 points in the first two epochs—followed by slower asymptotic convergence. A persistent optimization gap of 0.29 points remains at convergence (2.54 vs.\ 2.25 for baseline), consistent with the reduced expressivity of the constrained parameter space. This gap reflects a deliberate and quantifiable trade-off between architectural flexibility and structural regularity enforced by the non-negativity constraints.
