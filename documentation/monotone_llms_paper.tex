...
\section{Conclusion}
We investigated monotonicity as a structural inductive bias for improving the robustness of sequence-to-sequence language models. By enforcing non-negativity constraints on feed-forward sublayers in Transformer architectures, we observed substantial reductions in vulnerability to gradient-based adversarial attacks (69.8 percent reduction in HotFlip attack success rate), with only modest impact on summarization quality (3.2 percent ROUGE-L decrease). Results were verified across three random seeds (42, 1337, 2024), demonstrating consistency of the robustness improvements. These results suggest that simple architectural constraints can meaningfully influence robustness without altering attention mechanisms or relying on adversarial training.
