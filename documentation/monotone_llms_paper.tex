...
Table~\ref{tab:rouge} reports summarization performance on CNN/DailyMail (200 test examples, seed 42). The monotonic model achieves ROUGE-L of 24.2 [22.7, 25.6], compared to 25.0 [23.6, 26.4] for the baseline, corresponding to a relative decrease of 3.2 percent. ROUGE-1 shows a similar pattern (29.6 vs 30.9, a 4.2 percent relative decrease). While these results indicate a modest reduction in task performance, the monotonic model remains competitive, with confidence intervals overlapping for ROUGE-2 and ROUGE-L, suggesting the performance gap may not be statistically significant for all metrics.
